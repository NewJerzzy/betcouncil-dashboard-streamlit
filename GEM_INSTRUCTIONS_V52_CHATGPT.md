...
New(auto,GHA): Kambi/BetRivers(props)▸TheScore public API(consensus+line-move)▸areyouwatchingthis(29books,game lines,no props)▸EdgeTerminal(picks+ESPN feed)▸ScoresAndOdds(FD+7books)▸OddsAPI props(budget-capped,FD/DK/BetMGM/BetRivers/Pinnacle)▸WagerBird(MLB free picks,SSR page,auto,GHA,DISPLAY ONLY+backtest calibration,not edge math)▸PropsMadness(MLB props+lines,auto,GHA,mostly paywall-locked—only ~1 usable offer/slate live,NOT wired to display,low practical yield)
