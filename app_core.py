...
def save_to_gist(data_type, data, force_flush=True, suppress_window_flush=False):
    """
    ...
    suppress_window_flush=True additionally skips the batch_age window-expiry
    trigger below, not just the force_flush one. Real, confirmed gap in the
    fix above: with force_flush=False alone, a bulk-submit loop taking longer
    than _GIST_BATCH_WINDOW (5s) could still trigger a real, slow mid-loop
    network flush purely from the window expiring -- explaining reports of
    Submit All Parsed Bets hanging/needing multiple clicks with several bets
    queued. Only pass True when the caller guarantees a real flush happens
    before the run ends, same contract as force_flush=False.
    """
...
def record_signal_performance(lock, outcome, defer_gist_flush=False):
    ...
    save_to_gist("signal_performance", performance, force_flush=not defer_gist_flush, suppress_window_flush=defer_gist_flush)
            ...
            if fair_total is not None:
                ...
                if sport in ("MLB", "NHL", "Soccer") and _mu_h and _mu_a and abs(total_edge) >= 0.15:
                    try:
                        ...
                        _skellam_prob = compute_fair_prob_total(total_val, _mu_h, _mu_a, _skellam_side)
...
def build_market_comparison(shortlist):
    ...
    """
    result = {"dk_props": [], "fd_parlayhub": {}, "favoredprops": [], "bettingpros": [], "covers": [], "dimers": [], "draftedge": [], "mybookie": [], "vegasinsider": [], "sportsinsights": [], "scoresandodds": [], "pickswise": [], "actionnetwork": [], "betql": [], "propsmadness": []}
    ...
    # ── shortlist_matchups needed below for BettingPros + Covers ────────
...
def log_manual_bet(player, prop, line, side, sport, outcome, wager, pick_count, bet_type, source, bet_date, tier=None, edge=None, prob=None, notes="", signals=None, clv_capture=None, defer_gist_flush=False):
    ...
    save_to_gist("history", st.session_state.get("history", []), force_flush=not defer_gist_flush, suppress_window_flush=defer_gist_flush)
    ...
    save_to_gist("bankroll", st.session_state.get("bankroll", DEFAULT_BANKROLL), force_flush=not defer_gist_flush, suppress_window_flush=defer_gist_flush)
...
def load_sport_data(sport):
    ...
    def _pf_parlayapi():
        if _parlayapi_key in st.session_state:
            result = st.session_state[_parlayapi_key]
        else:
            result = fetch_parlayapi_props(sport)
            if result:
                st.session_state[_parlayapi_key] = result
        if result:
            st.session_state["parlayapi_props_cache"] = result
        return result
    ...
    def _pf_bobbys_briefing():
        _bb_sport_map = {"MLB": "mlb", "NBA": "nba", "WNBA": "wnba", "NFL": "nfl", "NHL": "nhl"}
        _bb_slug = _bb_sport_map.get(sport)
        return fetch_bobbys_bets_briefing(_bb_slug) if _bb_slug else {}
    def _pf_bobbys_scoreboard():
        return fetch_bobbys_bets_scoreboard("mlb") if sport == "MLB" else []
    def _pf_bobbys_best_prices():
        _bb_sport_map = {"MLB": "mlb", "NBA": "nba", "WNBA": "wnba", "NFL": "nfl", "NHL": "nhl"}
        _bb_slug = _bb_sport_map.get(sport)
        return fetch_bobbys_bets_best_prices(_bb_slug) if _bb_slug else {}
    ...
    def _pf_ev_nfl_props():  return fetch_ev_nfl_props() if sport in ("NFL",) else {}
    def _pf_ev_nfl_backfields(): return fetch_ev_nfl_backfields() if sport in ("NFL",) else {}
    ...
    def _get_parallel_results(_cache_sport):
        ...
        _pf_ev_stats_hr, _pf_ev_stats_k, _pf_ev_barrels, _pf_ev_recap, _pf_ev_mlb, _pf_ev_trends, _pf_ev_nfl_tds, _pf_ev_nfl_backfields, _pf_ev_nfl_props,
    ...
    (pp_props, ud_props_compare, pinnacle_data,
     ...
     ev_stats_hr_raw, ev_stats_k_raw, ev_barrels_raw, ev_recap_raw, ev_mlb_raw, ev_trends_raw, ev_nfl_tds_raw, ev_nfl_backfields_raw, ev_nfl_props_raw,
    ...
    # Bovada game lines — all sports, no auth required (moved into parallel
    # batch above; was a sequential blocking call after the entire ~164s
    # enrichment pass)
