...
## SECTION: Sharp Signal Parsing — MODE A Brief
The MODE A brief includes sharp-money signal fields; apply these rules whenever present.

**Brief fields:** `🔥STEAM` = line moved ≥0.5pt at sharp books within 5 min (strongest sharp signal); `conf=X` = 0-1 confidence, >0.70 = high. `ParlayAPIEV` = ParlayAPI independently confirms +EV vs Pinnacle. `PublicPct` = % public tickets (Action Network). `SharpContrarian` = PublicPct<40% + Edge>3%. `PublicHeavy` = PublicPct>65% (square money warning). `⚡RLM_NOTE` = reverse line movement + strength/score. `[REGRESS:LOW/MED/HIGH]` = hot-streak regression risk. `CPOE:+X.X` = QB completion% over expected (passing props). `MKT_DIV` = market-maker divergence between setter (Pinnacle/BOL) and taker (DK/FD) lines. `SHARP_CONSENSUS:HIGH` = BOL+Pinnacle agree.

**Rules:**
- `🔥STEAM` present → raise confidence tier by 1 (APPROVED→ELITE, ELITE→SOVEREIGN). Single most reliable sharp indicator.
- RLM: STRONG (score ≥1.4) = primary confirmation signal (edge_mult already applied). MODERATE (0.9-1.4) = supporting only. WEAK = flag only, no boost.
- `MKT_DIV` ≥0.75pt `[STRONG]` → bet toward the setter line, not taker — sharp money hasn't been arbitraged yet.
- `SHARP_CONSENSUS:HIGH` → ×1.10 edge multiplier already applied; cite BOL/Pinnacle consensus in reasoning.
- `[REGRESS:MEDIUM]` on an OVER → caution note, reduce Kelly 20%. `[REGRESS:HIGH]` → downgrade tier 1, explicit warning. Never suppress UNDER props for regression (regression supports UNDER).
- `CPOE >+2.0` → lean OVER passing yards/TDs. `<-2.0` → lean UNDER. Within ±2.0 → neutral, use other signals.
- When steam appears on a game total, it affects every counting prop in that game (Ks, hits, etc.) — steam UP → lean OVER on those props; steam DOWN → lean UNDER. Lead analysis with steam moves before individual props.
...
- Already evaluated and declined — do not re-investigate: BallparkPal (license-restricted, redundant), Sportradar (enterprise-priced, redundant), Doink Sports / SportBot AI / Linemate.io (ToS or auth-restricted), SportsGameOdds / PropCruncher / DataStreak / OpticOdds (not integrated, no case made), EVSharps Preseason/Futures (redundant/off-use-case), WhaleAlerts Discord consensus (thin MLB/NFL volume, unproven track record). PickFinder.app is owner-authorized but should not be added as a duplicate live prop feed; consider only narrow gap-fill fields or middle-detection inputs that BetCouncil does not already provide.

### Engineering Audit Addendum (Sept 2026)
- Fixed a real naming collision: `track_closing_line_beat` was defined twice in bc_utils.py with different, incompatible signatures (confirmed zero real callers for either, so both renamed safely — the dead one is now `compute_closing_line_beat_simple`).
- Swept `st.session_state.KEY` unsafe attribute reads (locks/bankroll/history/board_data/games/etc) to the safe `.get()` pattern across app.py and app_core.py — same bug class that caused 2 confirmed live crashes this session, now fixed comprehensively.
- Removed `attached_assets/` (191 dead files — screenshots, zips, pasted-text snippets from prior sessions, zero real references anywhere).
- Added a real, self-cleaning Gist job (`scripts/gist_self_cleanup.py`, daily cron) — prunes any Gist file untouched 30+ days, with a two-run confirmation before deleting anything and a durable, reviewable deletion log. No more manual cleanup needed for orphaned harvester output.
- **Standing rule going forward**: new `fetch_*` functions should default to `@st.cache_data(ttl=...)` rather than hand-rolled pkl/Gist caching boilerplate, unless there's a specific reason the manual pattern is needed (e.g. cross-redeploy persistence via Gist, which `st.cache_data` alone doesn't provide). Existing functions don't need retrofitting on sight — this is about new code going forward, not a rewrite mandate.
