...
def push_files(files_payload: dict) -> int:
    ...
    the real distributed lock. Real, confirmed-used source
    (fetch_covers_from_gist scraper-fallback path).
    """
    github_token = os.environ["GITHUB_TOKEN"]
    if not _rate_limit_ok(github_token):
        return 0
    SHARED_FILE = "betcouncil_covers_combined.json"
    merged = {}
    for fname, fbody in files_payload.items():
        key = fname.replace("betcouncil_covers_", "").replace(".json", "")
        try:
            merged[key] = json.loads(fbody["content"])
        except Exception:
            merged[key] = fbody["content"]

    lock_token = acquire_lock(GIST_ID, github_token, "covers_combined", holder="covers", max_attempts=7)
    if not lock_token:
        log("Could not acquire covers_combined lock -- skipping this run to avoid a collision")
        return 0
    try:
        try:
        ...
        release_lock(GIST_ID, github_token, "covers_combined", lock_token)
