...
try:
    _rc = main()
    _diag["result"] = f"main() returned {_rc}"
except Exception:
    _diag["exception"] = traceback.format_exc()
_diag["debug_log_tail"] = DEBUG_LOG[-10:]
_tok = os.environ.get("GITHUB_TOKEN")
requests.patch(f"https://api.github.com/gists/{GIST_ID}",
    headers={"Authorization": f"Bearer {_tok}", "Accept": "application/vnd.github+json"},
    json={"files": {"betcouncil_dfsr_props_debug.json": {"content": json.dumps(_diag, default=str)}}})
