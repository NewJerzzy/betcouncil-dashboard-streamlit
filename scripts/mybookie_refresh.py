...
def push_files(files_payload: dict, github_token: str) -> int:
    """
    Merges all mybookie_ssr_{sport} files into ONE file
    (betcouncil_mybookie_ssr_combined.json) instead of ~6 separate
    files. Uses the real distributed lock since this now writes to
    one shared file.
    """
    SHARED_FILE = "betcouncil_mybookie_ssr_combined.json"
    merged = {}
    for fname, fbody in files_payload.items():
        key = fname.replace("betcouncil_mybookie_ssr_", "").replace(".json", "")
        try:
            merged[key] = json.loads(fbody["content"])
        except Exception:
            merged[key] = fbody["content"]

    lock_token = acquire_lock(GIST_ID, github_token, "mybookie_ssr_combined", holder="mybookie")
    if not lock_token:
        log("Could not acquire mybookie_ssr_combined lock -- skipping this run to avoid a collision")
        return 0
    try:
        try:
        ...
        for attempt in range(4):
            ...
            if resp.status_code in (403, 429, 409) and attempt < 3:
                ...
                log(f"Gist push got {resp.status_code} -- retrying in {wait:.1f}s (attempt {attempt+1}/4)")
                time.sleep(wait)
                continue
        ...
        release_lock(GIST_ID, github_token, "mybookie_ssr_combined", lock_token)
