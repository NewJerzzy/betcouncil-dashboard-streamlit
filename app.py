...
SPORT_SOURCE_MAP = {
    "NBA":    ["FantasyPros", "OddsTrader", "SportsBettingDime"],
    "MLB":    ["FantasyPros", "OddsTrader", "SportsBettingDime"],
...
if "site_status" not in st.session_state:
    st.session_state.site_status = {
        ...
        for name in list(PROP_SOURCES.keys()) + list(GAME_SOURCES.keys()) + list(CONSENSUS_SOURCES.keys())
    }
...
def calculate_lock_integrity(best_prop, best_game, board):
    scores = [best_prop.get("Weighted Score", 0)]
...
def generate_full_summary(sport, board, game_board, raw_games, raw_games_for_summary):
    ...
    for name in list(PROP_SOURCES.keys()) + list(GAME_SOURCES.keys()) + list(CONSENSUS_SOURCES.keys()):
        ...
        if s == "ok": active_sources.append(f"✅ {name}")
        else: active_sources.append(f"⚪ {name}")
    ...
    total_raw = st.session_state.raw_props_total
    if total_raw <= 5:
        pi_score, pi_desc = 50, "Limited Data"
    else:
        pi_score, pi_desc = calculate_prop_integrity(seen_count, total_raw)
    ...
    lines.append("---")
    lines.append("")
    lines.append("## 📊 PLAYER PROPS (What the Scrapers Found — {0} survived dedup)".format(seen_count))
...
def parse_vsin_splits(html, sport):
    ...
    for row in rows:
        ...
        if len(pcts) >= 2 and len(teams_found) >= 2:
            away_team = teams_found[0]; home_team = teams_found[1] if len(teams_found) > 1 else teams_found[0]
            away_pct = int(pcts[0]); home_pct = int(pcts[1])
            sharp = "BALANCED"
            if away_pct >= 65: sharp = f"PUBLIC on {away_team} ({away_pct}%) — potential fade"
            elif home_pct >= 65: sharp = f"PUBLIC on {home_team} ({home_pct}%) — potential fade"
            elif abs(away_pct - home_pct) <= 10: sharp = "BALANCED — no clear edge"
            results.append({"matchup": f"{away_team} @ {home_team}", "away_pct": away_pct, "home_pct": home_pct, "sharp_signal": sharp, "sport": sport})
    return results
...
with tabs[2]:
    ...
    st.markdown(f"**Sources:** FantasyPros · OddsTrader · SportsBettingDime · ESPN · VSIN")
    st.markdown("---")
...
with tabs[6]:
    ...
    st.markdown("---"); st.markdown("## Site Health")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        for name in list(GAME_SOURCES.keys()) + list(CONSENSUS_SOURCES.keys()):
            s = st.session_state.site_status.get(name, {}).get("status", "unknown"); st.markdown(f":{dot(s)}[●] **{name}**")
