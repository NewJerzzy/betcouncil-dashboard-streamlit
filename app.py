...
# DraftKings removed entirely — JS-only, confirmed 0 results every scan
PROP_SOURCES = {
    "BettingPros": True,
    "OddsTrader": True,
...
# ──────────────────────────────────────────────────────────────
# URL Builder (UPDATED — per-source conditional)
# ──────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────
# HTML Parser — Site-Specific (THE FIX)
# ──────────────────────────────────────────────────────────────
...
def parse_espn_json(html, sport):
    """Parse ESPN JSON API response into game matchups."""
    out = []
    try:
        js = json.loads(html)
...
# ──────────────────────────────────────────────────────────────
# Main Sport Data Loader (UPDATED)
# ──────────────────────────────────────────────────────────────
    ...
    for gs_name in ["VegasInsider", "ESPN (JSON API)"]:
        ...
        if html:
            if "ESPN" in gs_name:
                ...
                games = [{"Matchup": g.get("matchup", ""), "Spread": g.get("spread", "N/A"), "Total": g.get("total", "N/A"), "Sport": sport} for g in parsed]
            if games:
                all_games = games
                break
...
sources_fail = sum(1 for v in st.session_state.site_status.values() if v.get("status") == "fail")
total_sources = max(len(st.session_state.site_status), 1)
recent_check = bool(st.session_state.last_scan_time)

firewall_checks = {
    "All core sources online": sources_ok >= max(total_sources - 2, 1),
