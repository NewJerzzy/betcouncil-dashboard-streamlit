...
# Game line sources — VegasInsider replaced with TeamRankings (server-rendered, confirmed working)
GAME_SOURCES = {
...
# Sport-specific source lists — DraftKings removed from all
SPORT_SOURCE_MAP = {
...
# =========================
# SITE-SPECIFIC PARSERS (FIXED)
# =========================
def clean_team_name(name):
    """Strip leading row IDs and trailing junk from team names."""
    name = re.sub(r'^\d+', '', name).strip()
    name = re.sub(r'[+\-]\d{3,}$', '', name).strip()
    return name

def parse_props_generic(html, source_name=""):
    """Site-specific parsers using correct CSS selectors for each source."""
...
def run_game_council_on_games(raw_games):
    ...
    for game in raw_games:
        ...
        for m in MODELS:
            name = m["name"]
            votes[name] = 1 if any(t in matchup for t in ["CLE", "OKC", "NYY", "PHI", "CHC", "NYK", "SAS", "Knicks", "Cavaliers", "Thunder", "Lakers", "@"]) else 0
        ws = weighted_score(votes)
        tier = get_tier(ws)
        results.append({
