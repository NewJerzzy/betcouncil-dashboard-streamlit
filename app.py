...
# Game line sources — TeamRankings (server-rendered) + ESPN failover
GAME_SOURCES = {
...
def build_source_url(source_name, sport):
    ...
    tr_sport = TR_SPORT_SLUG.get(sport, sport_lower)
    ...
    elif source_name == "TeamRankings":
        return f"https://www.teamrankings.com/{tr_sport}/odds/"
...
def parse_props_generic(html, source_name=""):
    ...
    # --- TEAMRANKINGS ---
    if source_name == "TeamRankings":
        rows = soup.select("table.odds-table tr, table.tr-table tr")
        for row in rows:
            cols = row.select("td")
            if len(cols) >= 4:
                matchup = clean_team_name(cols[0].get_text(strip=True))
    ...
    # --- VEGASINSIDER (fallback) ---
        ...
        for row in rows:
            ...
            if len(cols) >= 3:
                matchup = clean_team_name(cols[0].get_text(strip=True))
                spread_raw = cols[1].get_text(strip=True)
                total_raw = cols[2].get_text(strip=True) if len(cols) > 2 else ""
                spread_clean = spread_raw
                spread_match = re.match(r'([+-]?\d+\.?\d*)([+-]\d{3})', spread_raw)
                if spread_match:
                    spread_clean = spread_match.group(1)
                total_clean = total_raw
                if 'o' in total_raw.lower() or 'u' in total_raw.lower():
                    total_match = re.match(r'([ou])(\d+\.?\d*)([+-]\d{3})', total_raw, re.I)
                    if total_match:
                        total_clean = f"{total_match.group(1).upper()} {total_match.group(2)}"
                if matchup:
                    results.append({"matchup": matchup, "spread": spread_clean, "total": total_clean})
    ...
    # --- ODDSTRADER ---
        ...
        for card in soup.select(".prop-card, .player-prop-card, .market-card"):
            ...
            if player:
                results.append({
                    "player": clean_team_name(player.get_text(strip=True)),
    ...
    # --- GENERIC FALLBACK ---
        ...
        for table in soup.find_all("table"):
            for row in table.find_all("tr"):
                ...
                if len(cols) >= 2:
                    results.append({
                        "col1": clean_team_name(cols[0].get_text(strip=True)),
...
def run_game_council_on_games(raw_games):
    """Expand each game into ML, Spread, Over, Under bets and run Council."""
    ...
    for game in raw_games:
        ...
        # Generate bet lines from this game
        bets = []
        teams = matchup.split("@") if "@" in matchup else matchup.split(" at ")
        ...
        for bet in bets:
            ...
            for m in MODELS:
                name = m["name"]
                # Simple heuristic: home favorites get approval
                home_fav = "home" in bet_name.lower() or any(t in bet_name for t in ["Knicks", "Cavaliers", "Thunder", "Lakers", "Celtics", "Nuggets"])
                votes[name] = 1 if home_fav or "Under" in bet_name else 0
...
def load_sport_data_live(sport):
    ...
    for gs_name in ["TeamRankings", "ESPN (JSON API)"]:
        ...
        if html:
            if "ESPN" in gs_name:
                ...
                games = [{"Matchup": g.get("matchup", ""), "Spread": g.get("spread", ""), "Total": g.get("total", ""), "Sport": sport} for g in parse_props_generic(html, gs_name)]
            if games:
                all_games_raw = games
...
with st.sidebar:
    ...
    <div style="text-align:center;margin-bottom:16px;">
        ...
        <div style="font-size:11px;color:#4a8a8a;margin-top:2px;">v3.2 · TeamRankings</div>
    </div>
    """, unsafe_allow_html=True)
...
with tabs[1]:
    ...
    st.markdown("**Sources:** BettingPros · OddsTrader · SportsBettingDime · TeamRankings · ESPN")
    st.markdown("---")
